...
# Tipo de registo: True para certificado, False para registro normal
...
def registar_produtor():
    if REGISTRO_COM_CERTIFICADO:
        registar_produtor_com_certificado()
    else:
...
def carregar_chave_publica_arquivo(caminho_arquivo):
    # Abrir e ler o conteúdo do arquivo PEM
    with open(caminho_arquivo, "rb") as f:
        chave_publica_gestor_pem = f.read()
    return chave_publica_gestor_pem


def registar_produtor_com_certificado():

    # Chamar a função para carregar a chave
    caminho_arquivo = r"C:\Users\Utlizador\Desktop\Trabalhos\FSD(Python)\Fase3\manager_public_key.pem"
    chave_publica_gestor_pem = carregar_chave_publica_arquivo(caminho_arquivo)

    url = "http://193.136.11.170:5001/produtor_certificado"
    dados = {
        "ip": "127.0.0.1",  # IP do produtor
        "porta": 5000,  # Porta do produtor
        "nome": "Produtor_A",  # Nome do produtor
        "pubKey": chave_publica_gestor_pem.decode('utf-8')  
    }

    response = requests.post(url, json=dados)
    if response.status_code in [200, 201]:  # Verifica se o status é 200 ou 201
        print("Produtor registado com sucesso")
        # Salvar o certificado recebido
        certificado = response.content
        with open("certificado_produtor.pem", "wb") as f:
            f.write(certificado)
    else:
        print(f"Erro ao registar o produtor. Status code: {response.status_code}")



if __name__ == "__main__":
    if REGISTRO_COM_CERTIFICADO:
        print("A iniciar com registro certificado...")
    else:
        print("A iniciar com registro normal...")
